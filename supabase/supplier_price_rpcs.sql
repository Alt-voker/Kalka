...
declare
  ...
  v_supplier_id uuid;
begin
  select spl.organization_id, spl.supplier_id into v_org_id, v_supplier_id
  from public.supplier_price_lists spl
  where spl.id = target_price_list_id
  limit 1;
  if v_org_id is null then
  ...
  with incoming as (
    select
      ...
      nullif(trim(coalesce(item->>'raw_row', '{}')), '')::jsonb as raw_row,
      coalesce(nullif(trim(coalesce(item->>'row_index', '')), ''), null) as row_index_text,
      nullif(trim(coalesce(item->>'status', 'active')), '') as status_text,
      coalesce(nullif(replace(replace(replace(trim(coalesce(item->>'price', '')), '₽', ''), 'р', ''), ' ', ''), ''), null) as price_text
  ...
  select
    ...
    unit,
    nullif(replace(price_text, ',', '.'), '')::numeric as price,
    ...
    case when lower(coalesce(status_text, 'active')) in ('active', 'inactive', 'archived', 'deleted') then lower(coalesce(status_text, 'active')) else 'active' end as status,
    now(),
    now()
  from incoming
  where coalesce(raw_name, '') <> ''
    and nullif(replace(price_text, ',', '.'), '') is not null
    and nullif(replace(price_text, ',', '.'), '')::numeric > 0
  returning *;
end;
$$;
