(function (window) {
  ...
  function normalizeSessionRow(data) {
    ...
    var permissions = normalizePermissionList(row.permissions || row.permission_keys || []);
    var activeOrganizationPermissions = normalizePermissionList(row.activeOrganizationPermissions || row.active_organization_permissions || permissions);
    return {
      row: row,
      profileId: profileId,
      ...
      activeOrganizationPermissions: activeOrganizationPermissions,
  ...
  async function bootstrapSession(authUser) {
    ...
    session.permissionKeys = Array.isArray(normalized.permissions) ? normalized.permissions.slice() : [];
    session.activeOrganizationPermissions = Array.isArray(normalized.activeOrganizationPermissions)
      ? normalized.activeOrganizationPermissions.slice()
      : session.permissionKeys.slice();
