...
const PM={
  ...
  restaurants:    {sec:'Управление',  ico:'', lbl:'Организации'},
...
var _supPriceManagerTab = 'lists';
var _supPriceCatalogRows = [];
var _supPriceCatalogLoading = false;
...
function getPageRenderer(pg){
  return {
    ...
    'chef-calc': initCalc,
    'sup-products': renderSupProducts,
    'sup-orders': renderSupOrders,
    'sup-analytics': renderSupAnalytics,
    'sup-dashboard': renderSupDash,
    admin: renderAdmin,
...
function ensureSupplierDetailsModal(){
  ...
  el.id='ov-supDetails';
  el.innerHTML=''
    +'<div class="modal xl" style="max-width:920px;width:min(920px,calc(100vw - 24px));">'
      +'<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">'
        +'<div><div class="m-title" id="supDetailsTitle">Поставщик</div><div id="supDetailsMeta" style="font-size:12px;color:var(--t2);margin-top:6px;"></div></div>'
        +'<button class="tbBtn" onclick="closeSupplierDetailsModal()" style="cursor:pointer;">Закрыть</button>'
      +'</div>'
...
function openSupplierDetailsModal(index){
  ...
  var title=document.getElementById('supDetailsTitle');
  var meta=document.getElementById('supDetailsMeta');
  var body=document.getElementById('supDetailsBody');
  var legalNames = Array.isArray(supplier.legalEntityNames) && supplier.legalEntityNames.length ? supplier.legalEntityNames.join(', ') : (Array.isArray(supplier.legal_entity_names) && supplier.legal_entity_names.length ? supplier.legal_entity_names.join(', ') : (supplier.legalName || '—'));
  if(title) title.textContent = supplier.name || 'Поставщик';
  if(meta) meta.textContent = 'Статус: ' + (supplier.status || 'active');
  if(body){
    body.innerHTML=''
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">Название</div><div style="font-size:15px;font-weight:800;margin-top:6px;">'+_esc(supplier.name || '—')+'</div>'
      +'</div>'
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">ИНН</div><div style="font-size:15px;font-weight:800;margin-top:6px;">'+_esc(supplier.inn || '—')+'</div>'
      +'</div>'
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">Контакт</div><div style="font-size:15px;font-weight:800;margin-top:6px;">'+_esc(supplier.contactName || supplier.contact_name || supplier.contact || '—')+'</div>'
      +'</div>'
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">Телефон / Email</div><div style="font-size:15px;font-weight:800;margin-top:6px;">'+_esc(supplier.phone || '—')+' / '+_esc(supplier.email || '—')+'</div>'
      +'</div>'
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">Юрлица</div><div style="font-size:15px;font-weight:800;margin-top:6px;">'+_esc(legalNames)+'</div>'
      +'</div>';
  }
...
function normalizeSupplierOrganizationIds(supplier, db){
  ...
  if(supplier.organization_id) ids.push(String(supplier.organization_id));
  if(supplier.organizationId) ids.push(String(supplier.organizationId));
...
function getSupplierVisibleOrganizationsForUser(supplier, user, db){
  ...
  var activeOrgId=String((window.__userSession && window.__userSession.activeOrganizationId) || '').trim();
  var org = activeOrgId ? getOrganizationById(activeOrgId) : null;
  if (!org && activeOrgId && window.activeRest && String(window.activeRest.id || window.activeRest.organizationId || '') === activeOrgId) {
...
function getUserVisibleSuppliers(user){
  var db=dbGet();
  var activeOrgId=String((window.__userSession && window.__userSession.activeOrganizationId) || '').trim();
  var base=(window.__userSession && Array.isArray(window.__userSession.suppliers) ? window.__userSession.suppliers : (SUPS_DATA||[])).filter(function(s){
    if(!s || s.hidden) return false;
    if(!activeOrgId) return true;
    var ids=normalizeSupplierOrganizationIds(s, db);
    return ids.indexOf(activeOrgId)>=0;
  });
  if(!user) return [];
...
function openModal(name){
  ...
  if(name==='assignUserOrg' && typeof window.openAssignUserToOrganizationModal === 'function') return;
...
function normalizeSupplierLegalEntityIds(supplier){
  if(!supplier) return [];
  var ids=[];
  if(Array.isArray(supplier.legalEntityIds)) ids=ids.concat(supplier.legalEntityIds);
  if(Array.isArray(supplier.legal_entity_ids)) ids=ids.concat(supplier.legal_entity_ids);
...
function renderSupplierOrganizationOptions(selectedIds){
  ...
  box.innerHTML='<div style="display:grid;gap:6px;padding:10px;border:1px solid var(--br);border-radius:8px;background:var(--bg3);">'
    +'<div style="font-size:12px;color:var(--t3);text-transform:uppercase;letter-spacing:1px;">Текущая организация</div>'
    +'<div style="font-size:14px;font-weight:700;color:var(--tx);">'+(org.name||org.organization_name||'Организация без названия')+'</div>'
    +'<div style="font-size:11px;color:var(--t3);">'+((org.city||'') + ((org.address||org.legal_address) ? ' · '+(org.address||org.legal_address) : ''))+'</div>'
    +'<input type="hidden" id="as-org-id" value="'+_esc(orgId)+'">'
  +'</div>';
}
function closeSupplierModal(){
  ['as-edit-id','as-n','as-em2','as-legal','as-inn','as-city','as-ct','as-ph','as-em','as-dl','as-wh','as-mn','as-status'].forEach(function(id){
    var el=document.getElementById(id);if(el)el.value='';
  });
  var legalBox=document.getElementById('as-legal-entities'); if(legalBox) legalBox.innerHTML='';
...
function openSupplierModal(index, presetRestId){
  ...
  setVal('as-inn', supplier?supplier.inn:'');
...
function normalizeLegacyRoleSafe(role) {
  var normalizeRoleSafe =
    window.normalizeRole ||
    window.normalizeLegacyRole ||
    function(value) {
  ...
  return normalizeRoleSafe(role);
}

function getOrganizationRoleLabelSafe(role) {
  var normalized = String(normalizeLegacyRoleSafe(role) || '').trim();
  if (normalized === 'owner') return 'Владелец';
  if (normalized === 'admin') return 'Администратор';
  if (normalized === 'organization_owner') return 'Управляющий организации';
  if (normalized === 'manager') return 'Менеджер';
  if (normalized === 'buyer') return 'Закупщик';
  if (normalized === 'chef') return 'Шеф-повар';
  if (normalized === 'bar_manager') return 'Бар-менеджер';
  if (normalized === 'accountant') return 'Бухгалтер';
  if (normalized === 'warehouse') return 'Склад';
  return normalized || 'Не назначена';
}

function getOrganizationTypeLabelSafe(type) {
  var t = String(type || '').toLowerCase().trim();
  if (t === 'restaurant' || t === 'ресторан') return 'Ресторан';
  if (t === 'bar' || t === 'бар') return 'Бар';
  if (t === 'cafe' || t === 'кафе' || t === 'coffee' || t === 'кофейня') return 'Кафе';
  return 'Другое';
}

function getOrganizationStatusLabelSafe(status) {
  var normalized = String(status || 'active').toLowerCase().trim();
  if (normalized === 'active') return 'Активна';
  if (normalized === 'archived') return 'В архиве';
  if (normalized === 'deleted') return 'Удалена';
  return 'Не активна';
}

function getOrganizationMembersCountFromCache(orgId) {
  ...
  if (cache && Array.isArray(cache.items)) {
    return cache.items.filter(function (item) {
...
function setOrganizationListFilter(filter){
  window.__orgListFilter = String(filter || 'active').trim() || 'active';
  if(typeof renderRestaurants === 'function') renderRestaurants();
}

function normalizeOrganizationForRender(org) {
  if (!org) return null;
  var id = String(org.id || org.organization_id || org.organizationId || '').trim();
  if (!id) return null;
  return {
    id: id,
    name: org.name || org.organization_name || org.brandName || org.legalName || 'Организация без названия',
    status: String(org.status || 'active').trim().toLowerCase() || 'active',
    type: org.type || org.kind || 'restaurant',
    role: org.role || '',
    city: org.city || '',
    address: org.address || '',
...
function filterOrganizationsForTab(orgs, orgFilter) {
  var filter = String(orgFilter || 'active').trim().toLowerCase() || 'active';
  var normalized = Array.isArray(orgs) ? orgs.map(normalizeOrganizationForRender).filter(Boolean) : [];
  if (!normalized.length) return [];
  if (filter === 'archived') {
    return normalized.filter(function (org) {
  ...
  if (filter === 'deleted') {
    return normalized.filter(function (org) {
      return String(org.status || 'active').toLowerCase() === 'deleted';
    });
  }
  if (filter === 'all') return normalized.slice();
  return normalized.filter(function (org) {
    return String(org.status || 'active').toLowerCase() === 'active';
  });
}

function getOrganizationsByCurrentFilterSafe(orgFilter, currentRole) {
  var session = window.__userSession || {};
  var orgs = Array.isArray(session.organizations) ? session.organizations.slice() : [];
  var filter = String(orgFilter || 'active').trim() || 'active';
  var sessionFiltered = filterOrganizationsForTab(orgs, filter);
  ...
  var cache = window.__dataCache && window.__dataCache.organizationsByFilter ? window.__dataCache.organizationsByFilter[filter] : null;
  if (cache && Array.isArray(cache.items)) return cache.items.slice();
  if (['owner', 'admin', 'organization_owner'].indexOf(normalizeLegacyRoleSafe(currentRole)) >= 0 && typeof window.loadOrganizationsForRestaurants === 'function') {
    window.loadOrganizationsForRestaurants(filter).catch(function () {});
  }
  return [];
}

function syncOrganizationListItemIntoSession(org) {
  if (!org || !window.__userSession) return;
  window.__userSession.organizations = Array.isArray(window.__userSession.organizations) ? window.__userSession.organizations : [];
  var merged = {
    id: String(org.id || '').trim(),
    name: org.name || 'Организация без названия',
  ...
  var idx = window.__userSession.organizations.findIndex(function (item) {
    return String(item && item.id || '').trim() === merged.id;
  });
  if (idx >= 0) {
    window.__userSession.organizations[idx] = Object.assign({}, window.__userSession.organizations[idx], merged);
  } else {
    window.__userSession.organizations.push(merged);
  }
  if (window.__userSession.activeOrganization && String(window.__userSession.activeOrganization.id || '').trim() === merged.id) {
    window.__userSession.activeOrganization = Object.assign({}, window.__userSession.activeOrganization, merged);
    window.__userSession.activeOrganizationId = merged.id;
    window.activeRest = {
      id: merged.id,
    ...
    if (String(merged.status || 'active').toLowerCase() !== 'active') {
      var nextActive = window.__userSession.organizations.find(function (item) {
        return item && String(item.status || 'active').toLowerCase() === 'active';
      }) || null;
      if (nextActive) {
        window.__userSession.activeOrganization = Object.assign({}, nextActive);
        window.__userSession.activeOrganizationId = nextActive.id;
        window.activeRest = {
          id: nextActive.id,
          organizationId: nextActive.id,
          name: nextActive.name || '',
          kind: nextActive.type || 'organization',
          type: nextActive.type || 'organization',
          members: []
        };
      }
    }
...
function renderRestaurants(){
  var pageContainer = document.querySelector('#pg-restaurants');
  if (!pageContainer) {
  ...
  console.info('organizations render data', {
    role: window.__userSession && window.__userSession.role,
    organizations: window.__userSession && window.__userSession.organizations ? window.__userSession.organizations.length : 0,
    activeOrganizationId: window.__userSession && window.__userSession.activeOrganizationId
  });
  ...
  try {
    ...
    console.error('renderRestaurants failed:', error);
    root.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--t2);background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:18px;box-shadow:0 10px 28px rgba(15,23,42,.08);">'
      +'<div style="font-size:18px;font-weight:700;margin-bottom:8px;">Не удалось отрисовать блок организаций</div>'
      +'<div style="font-size:13px;">Ошибка записана в консоль.</div>'
      +'</div>';
    return;
  }
  ...
  if (!root.__orgActionsBound) {
    root.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-org-action]') : null;
      if (!btn || !root.contains(btn)) return;
      e.preventDefault();
      e.stopPropagation();
      var action = String(btn.dataset.orgAction || '').trim();
      var orgId = String(btn.dataset.orgId || '').trim();
      if (!action || !orgId) return;
      console.info('organization action clicked', { action: action, orgId: orgId });
      try {
    ...
    root.__orgActionsBound = true;
  }
}

window.runOrganizationModuleSelfTest = async function () {
  var results = [];
...
function setActiveOrganization(orgId){
  if (window.__orgSwitchInProgress) return;
  window.__orgSwitchInProgress = true;
  var session = window.__userSession || {};
  try{
    logOrganizationAction('switch', orgId);
    ...
    if(typeof renderRestaurants === 'function') renderRestaurants();
    if(getCurrentPageId() !== 'restaurants' && typeof rerenderCurrentPage === 'function') rerenderCurrentPage();
  } catch(error){
    console.error('setActiveOrganization failed:', error);
    if (typeof window.toast === 'function') {
      window.toast('Не удалось переключить организацию', 'err');
    }
  } finally {
    window.__orgSwitchInProgress = false;
  }
}
...
function getOrganizationRoleLabel(role){
  ...
  if (normalized === 'platform_owner' || normalized === 'owner') return 'Владелец';
  if (normalized === 'admin') return 'Администратор';
  if (normalized === 'organization_owner') return 'Управляющий организации';
  if (normalized === 'manager') return 'Менеджер';
  if (normalized === 'buyer') return 'Закупщик';
...
function ensureOrganizationMembersModal(){
  var el=document.getElementById('ov-orgMembers');
  if(el) return el;
  el=document.createElement('div');
  el.className='ov';
  el.id='ov-orgMembers';
  el.innerHTML=''
    +'<div class="modal xl" style="max-width:960px;width:min(960px,calc(100vw - 24px));">'
      +'<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">'
        +'<div>'
          +'<div class="m-title" id="orgMembersTitle">👥 Участники организации</div>'
          +'<div id="orgMembersMeta" style="font-size:12px;color:var(--t2);margin-top:6px;"></div>'
        +'</div>'
        +'<button class="tbBtn" onclick="closeOrganizationMembersModal()" style="cursor:pointer;">Закрыть</button>'
      +'</div>'
      +'<div id="orgMembersStatus" style="margin-top:12px;font-size:12px;color:var(--t3);"></div>'
      +'<div id="orgMembersBody" style="display:grid;gap:10px;max-height:56vh;overflow:auto;margin-top:12px;"></div>'
      +'<div class="m-acts" style="margin-top:18px;">'
        +'<button class="m-cancel" onclick="closeOrganizationMembersModal()">Закрыть</button>'
      +'</div>'
    +'</div>';
  document.body.appendChild(el);
  return el;
}
function closeOrganizationMembersModal(){
  var el=document.getElementById('ov-orgMembers');
  if(el) el.classList.remove('on');
}
async function renderOrganizationMembersModal(orgId){
  var modal=ensureOrganizationMembersModal();
  var statusEl=document.getElementById('orgMembersStatus');
  var body=document.getElementById('orgMembersBody');
  var title=document.getElementById('orgMembersTitle');
  var meta=document.getElementById('orgMembersMeta');
  if(!body || !statusEl) return;
  var org=getOrganizationById(orgId);
  var orgName=org && org.name ? org.name : 'Организация без названия';
  var orgStatus=org && org.status ? org.status : 'active';
  if(title) title.textContent='👥 Участники · '+orgName;
  if(meta) meta.textContent='Статус организации: '+(String(orgStatus).toLowerCase()==='active' ? 'Активна' : 'Не активна');
  if(!orgId){
  ...
  if(bucket && bucket.loading && !Array.isArray(bucket.items)){
    ...
    if (typeof window.loadOrganizationMembersForOrganization === 'function' && !bucket.promise) {
      window.loadOrganizationMembersForOrganization(orgId).then(function () {
        renderOrganizationMembersModal(orgId);
      }).catch(function () {
        renderOrganizationMembersModal(orgId);
      });
    }
  ...
  if(!bucket || (!Array.isArray(bucket.items) && !bucket.promise)){
    statusEl.textContent='Загрузка участников...';
    body.innerHTML='<div style="padding:22px;text-align:center;color:var(--t3);background:var(--bg3);border:1px solid var(--br);border-radius:14px;">Загрузка участников...</div>';
    if (typeof window.loadOrganizationMembersForOrganization === 'function') {
      window.loadOrganizationMembersForOrganization(orgId).then(function () {
        renderOrganizationMembersModal(orgId);
      }).catch(function () {
        renderOrganizationMembersModal(orgId);
  ...
  if(bucket && bucket.error){
    console.error('organization members load failed', {
      orgId: String(orgId || ''),
    ...
    statusEl.innerHTML=''
      +'<span style="color:var(--rd);">Не удалось загрузить участников</span>'
      +'<div style="margin-top:6px;font-size:11px;color:var(--t3);white-space:pre-wrap;">'+_esc([bucket.error.message, bucket.error.details, bucket.error.hint].filter(Boolean).join(' · ') || 'Ошибка запроса участников')+'</div>'
      +'<button class="tbBtn" onclick="refreshOrganizationMembersForOrganization(\''+String(orgId).replace(/'/g, '\\\'')+'\')" style="cursor:pointer;margin-left:8px;margin-top:8px;">Повторить</button>';
    body.innerHTML='<div style="padding:22px;text-align:center;color:var(--t3);background:var(--bg3);border:1px solid var(--br);border-radius:14px;">Не удалось загрузить участников</div>';
    return;
  }
  ...
  body.innerHTML=items.map(function (member) {
    ...
    var statusStyle = String(member.status || 'active').toLowerCase() === 'active'
      ? 'background:var(--grD);color:var(--gr);border:1px solid var(--gr);'
      : 'background:#eef2f7;color:#475569;border:1px solid #cbd5e1;';
    return ''
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;box-shadow:0 8px 20px rgba(15,23,42,.06);padding:14px 16px;">'
        +'<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;">'
          +'<div style="min-width:0;">'
            +'<div style="font-size:15px;font-weight:800;word-break:break-word;">'+_esc(displayName)+'</div>'
            +'<div style="font-size:12px;color:var(--t3);margin-top:4px;word-break:break-word;">'+_esc(member.email || '—')+'</div>'
            +'<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">'
              +'<span class="badge" style="background:#eef6ff;color:#2463eb;border:1px solid #cfe2ff;">'+_esc(roleLabel)+'</span>'
              +'<span class="badge" style="'+statusStyle+'">Статус: '+statusText+'</span>'
            +'</div>'
          +'</div>'
          +'<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;">'
            +(isReadonlyRole
              ? '<span class="badge" style="background:#eef6ff;color:#2463eb;border:1px solid #cfe2ff;">Роль владельца платформы</span>'
              : ''
            )
            +(isReadonlyRole ? '' : '<select class="fS" id="orgMemberRole-'+_esc(member.membershipId || member.userProfileId || member.authUserId || member.email)+'" style="margin:0;min-width:180px;">'
              +'<option value="admin" '+(String(member.rawRole || member.role) === 'admin' ? 'selected' : '')+'>Администратор</option>'
              +'<option value="organization_owner" '+(String(member.rawRole || member.role) === 'organization_owner' ? 'selected' : '')+'>Управляющий организации</option>'
              +'<option value="manager" '+(String(member.rawRole || member.role) === 'manager' ? 'selected' : '')+'>Менеджер</option>'
              +'<option value="buyer" '+(String(member.rawRole || member.role) === 'buyer' ? 'selected' : '')+'>Закупщик</option>'
              +'<option value="chef" '+(String(member.rawRole || member.role) === 'chef' ? 'selected' : '')+'>Шеф-повар</option>'
              +'<option value="bar_manager" '+(String(member.rawRole || member.role) === 'bar_manager' ? 'selected' : '')+'>Бар-менеджер</option>'
              +'<option value="accountant" '+(String(member.rawRole || member.role) === 'accountant' ? 'selected' : '')+'>Бухгалтер</option>'
              +'<option value="warehouse" '+(String(member.rawRole || member.role) === 'warehouse' ? 'selected' : '')+'>Склад</option>'
            +'</select>')
          +'</div>'
        +'</div>'
  ...
  if (!modal.__memberActionsBound) {
    modal.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-member-action]') : null;
      ...
      try {
        ...
        if (action === 'change-role') {
          var selectId = String(btn.dataset.roleSelectId || '').trim();
          var roleSelect = selectId ? document.getElementById(selectId) : null;
          var role = roleSelect ? String(roleSelect.value || '').trim() : '';
          if (typeof window.changeOrganizationMemberRole === 'function') {
            window.changeOrganizationMemberRole(targetUserProfileId, targetOrgId, role);
          }
          return;
        }
        if (action === 'disconnect') {
          if (typeof window.removeOrganizationMemberFromOrganization === 'function') {
            window.removeOrganizationMemberFromOrganization(targetUserProfileId, targetOrgId);
          }
          return;
        }
      } catch (error) {
        console.error('member action failed', { action: action, orgId: targetOrgId, error: error });
        if (typeof window.toast === 'function') window.toast('Не удалось выполнить действие', 'err');
      }
    });
    modal.__memberActionsBound = true;
  }
}
window.renderOrganizationMembersModal = renderOrganizationMembersModal;
window.openOrganizationMembersModal = function (orgId) {
  logOrganizationAction('members', orgId);
  var modal = ensureOrganizationMembersModal();
  modal.dataset.orgId = String(orgId || '').trim();
  modal.classList.add('on');
  renderOrganizationMembersModal(orgId);
};
window.changeOrganizationMemberRole = async function (userProfileId, orgId, role) {
  try {
    logOrganizationAction('change-role', orgId);
...
window.removeOrganizationMemberFromOrganization = async function (userProfileId, orgId) {
  try {
    logOrganizationAction('disconnect-member', orgId);
...
function ensureOrganizationAddMemberModal(){
  var el=document.getElementById('ov-orgMemberAdd');
  if(el) return el;
  el=document.createElement('div');
  el.className='ov';
  el.id='ov-orgMemberAdd';
  el.innerHTML=''
    +'<div class="modal" style="max-width:760px;width:min(760px,calc(100vw - 24px));">'
      ...
      +'</div>'
      +'<div id="orgMemberAddStatus" style="margin-top:12px;font-size:12px;color:var(--t3);"></div>'
      +'<div id="orgMemberAddBody" style="display:grid;gap:12px;margin-top:12px;"></div>'
      +'<div class="m-acts" style="margin-top:18px;">'
        +'<button class="m-ok" data-member-action="submit-add" onclick="submitOrganizationMemberAdd()">✅ Добавить</button>'
        +'<button class="m-cancel" onclick="closeOrganizationAddMemberModal()">Отмена</button>'
      +'</div>'
    +'</div>';
  document.body.appendChild(el);
  return el;
...
function closeOrganizationAddMemberModal(){
  var el=document.getElementById('ov-orgMemberAdd');
  if(el) el.classList.remove('on');
}

async function renderOrganizationAddMemberModal(orgId){
  var modal=ensureOrganizationAddMemberModal();
  var statusEl=document.getElementById('orgMemberAddStatus');
  var body=document.getElementById('orgMemberAddBody');
  var title=document.getElementById('orgMemberAddTitle');
  ...
  if((!Array.isArray(usersBucket.items) && !usersBucket.promise) || usersBucket.loading){
    ...
    body.innerHTML='<div style="padding:22px;text-align:center;color:var(--t3);background:var(--bg3);border:1px solid var(--br);border-radius:14px;">Загрузка пользователей...</div>';
    if(typeof window.loadOwnerUsers === 'function'){
      window.loadOwnerUsers().then(function(){ renderOrganizationAddMemberModal(orgId); }).catch(function(){ renderOrganizationAddMemberModal(orgId); });
    }
    return;
  }
  if(usersBucket.error){
    statusEl.innerHTML='<span style="color:var(--rd);">Не удалось загрузить пользователей</span> <button class="tbBtn" onclick="window.loadOwnerUsers().then(function(){ renderOrganizationAddMemberModal(\''+String(orgId).replace(/'/g, '\\\'')+'\'); }).catch(function(){});" style="cursor:pointer;margin-left:8px;">Повторить</button>';
    body.innerHTML='<div style="padding:22px;text-align:center;color:var(--t3);background:var(--bg3);border:1px solid var(--br);border-radius:14px;">Не удалось загрузить пользователей</div>';
    return;
  }
  var users = Array.isArray(usersBucket.items) ? usersBucket.items.slice() : [];
  if(!users.length){
    statusEl.innerHTML='<span>Пользователи не найдены</span> <button class="tbBtn" onclick="window.loadOwnerUsers().then(function(){ renderOrganizationAddMemberModal(\''+String(orgId).replace(/'/g, '\\\'')+'\'); }).catch(function(){});" style="cursor:pointer;margin-left:8px;">Повторить</button>';
    body.innerHTML='<div style="padding:22px;text-align:center;color:var(--t3);background:var(--bg3);border:1px solid var(--br);border-radius:14px;">Пользователи не найдены</div>';
    return;
  }
  var roleOptions = [
    { value:'admin', label:'Администратор' },
    { value:'organization_owner', label:'Управляющий организации' },
  ...
  body.innerHTML=''
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Пользователь</span>'
      +'<select id="orgMemberAddUser" class="fS" style="margin:0;">'
        +'<option value="">Выберите пользователя</option>'
        +users.map(function(user){
          var label = (user.first || user.first_name || '') + ' ' + (user.last || user.last_name || '');
          label = label.trim() || user.email || 'Пользователь';
          return '<option value="'+_esc(user.profileId || user.profile_id || user.id || user.auth_user_id || user.authUserId || '')+'">'+_esc(label + (user.email ? ' · ' + user.email : ''))+'</option>';
        }).join('')
      +'</select>'
    +'</label>'
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Роль</span>'
      +'<select id="orgMemberAddRole" class="fS" style="margin:0;">'
        +roleOptions.map(function(opt){ return '<option value="'+opt.value+'">'+opt.label+'</option>'; }).join('')
      +'</select>'
    +'</label>';
  window.__orgMemberAddOrgId = String(orgId || '').trim();
  if (!modal.__addMemberActionsBound) {
    modal.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-member-action="submit-add"]') : null;
      if (!btn || !modal.contains(btn)) return;
      e.preventDefault();
      e.stopPropagation();
      console.info('organization action clicked', { action: 'submit-add', orgId: String(window.__orgMemberAddOrgId || modal.dataset.orgId || '') });
      if (typeof window.submitOrganizationMemberAdd === 'function') {
        window.submitOrganizationMemberAdd();
      }
    });
    modal.__addMemberActionsBound = true;
  }
}

window.openOrganizationAddMemberModal = function (orgId) {
  var modal = ensureOrganizationAddMemberModal();
  modal.dataset.orgId = String(orgId || '').trim();
  modal.classList.add('on');
  renderOrganizationAddMemberModal(orgId);
};
window.closeOrganizationAddMemberModal = closeOrganizationAddMemberModal;

window.submitOrganizationMemberAdd = async function () {
  var orgId = String(window.__orgMemberAddOrgId || (document.getElementById('ov-orgMemberAdd') && document.getElementById('ov-orgMemberAdd').dataset.orgId) || '').trim();
  var userProfileId = String((document.getElementById('orgMemberAddUser') || { value: '' }).value || '').trim();
  var role = String((document.getElementById('orgMemberAddRole') || { value: 'manager' }).value || '').trim();
  try {
    logOrganizationAction('add-user', orgId);
    if (!orgId) throw new Error('Организация не выбрана');
    ...
    if (cache.ownerUsers && Array.isArray(cache.ownerUsers.items)) {
      user = cache.ownerUsers.items.find(function (item) {
        var profileId = String(item.profileId || item.user_profile_id || '').trim();
        var authUserId = String(item.authUserId || item.auth_user_id || '').trim();
        var id = String(item.id || '').trim();
        return profileId === userProfileId || authUserId === userProfileId || id === userProfileId;
      }) || null;
    }
...
function ensureOrganizationDetailsModal(){
  var el=document.getElementById('ov-orgDetails');
  if(el) return el;
  el=document.createElement('div');
  el.className='ov';
  el.id='ov-orgDetails';
  el.innerHTML=''
    +'<div class="modal xl" style="max-width:960px;width:min(960px,calc(100vw - 24px));">'
      +'<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">'
        +'<div>'
          +'<div class="m-title" id="orgDetailsTitle">🏢 Организация</div>'
          +'<div id="orgDetailsMeta" style="font-size:12px;color:var(--t2);margin-top:6px;"></div>'
        +'</div>'
        +'<button class="tbBtn" onclick="closeOrganizationDetailsModal()" style="cursor:pointer;">Закрыть</button>'
      +'</div>'
      +'<div id="orgDetailsBody" style="display:grid;gap:12px;margin-top:14px;"></div>'
      +'<div class="m-acts" style="margin-top:18px;">'
        +'<button class="m-cancel" onclick="closeOrganizationDetailsModal()">Закрыть</button>'
      +'</div>'
    +'</div>';
  document.body.appendChild(el);
  return el;
...
function logOrganizationAction(action, organizationId){
  console.info('organization action clicked', { action: String(action || ''), organizationId: String(organizationId || '') });
}

function openOrganizationDetailsModal(orgId){
  logOrganizationAction('open', orgId);
...
function renderOrganizationDetailsModal(orgId){
  var body=document.getElementById('orgDetailsBody');
  ...
  body.innerHTML=''
    +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">'
      ...
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;padding:14px 16px;box-shadow:0 8px 20px rgba(15,23,42,.06);">'
        ...
        +'<div style="font-size:15px;font-weight:800;margin-top:6px;">'+getOrganizationMembersLabelSafe(membersCount)+'</div>'
      +'</div>'
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;padding:14px 16px;box-shadow:0 8px 20px rgba(15,23,42,.06);">'
      ...
      +'<div style="background:#fff;border:1px solid rgba(148,163,184,.16);border-radius:16px;padding:14px 16px;box-shadow:0 8px 20px rgba(15,23,42,.06);">'
        ...
        +'<div style="font-size:15px;font-weight:800;margin-top:6px;">'+Number(activeMembersCount || 0)+'</div>'
      +'</div>'
    +'</div>'
    +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">'
      ...
      +'<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:14px 16px;">'
        ...
        +'<div style="font-size:14px;font-weight:700;margin-top:6px;">'+_esc(org.createdAt ? String(org.createdAt).slice(0, 10) : (org.created_at ? String(org.created_at).slice(0, 10) : '—'))+'</div>'
      +'</div>'
      +'<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:14px 16px;">'
        +'<div style="font-size:11px;color:var(--t3);text-transform:uppercase;">Рабочий контекст</div>'
    ...
    +'<div style="display:flex;gap:8px;flex-wrap:wrap;">'
      ...
      +((['owner','admin'].indexOf(currentRole) >= 0) ? (
        String(org.status || '').toLowerCase() === 'archived'
          ...
          : '<button class="tbBtn" data-org-action="archive" data-org-id="'+_esc(String(org.id))+'" style="cursor:pointer;">Архивировать</button>'
      ) : '')
      +((currentRole === 'owner')
        ? '<button class="tbBtn" data-org-action="delete" data-org-id="'+_esc(String(org.id))+'" style="cursor:pointer;">Удалить</button>'
        : '')
    +'</div>';
  if (!modal.__detailsActionsBound) {
    modal.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-org-action]') : null;
      if (!btn || !modal.contains(btn)) return;
      e.preventDefault();
      e.stopPropagation();
      var action = String(btn.dataset.orgAction || '').trim();
      var targetOrgId = String(btn.dataset.orgId || org.id || '').trim();
      console.info('organization action clicked', { action: action, orgId: targetOrgId });
      try {
        if (action === 'switch' && typeof window.setActiveOrganization === 'function') {
        ...
        if (action === 'archive' && typeof window.archiveOrganizationFromCard === 'function') {
          window.archiveOrganizationFromCard(targetOrgId);
          return;
        }
        if (action === 'restore' && typeof window.restoreOrganizationFromCard === 'function') {
          window.restoreOrganizationFromCard(targetOrgId);
          return;
        }
        if (action === 'delete' && typeof window.deleteOrganizationFromCard === 'function') {
          window.deleteOrganizationFromCard(targetOrgId);
          return;
        }
      } catch (error) {
        console.error('organization details action failed', { action: action, orgId: targetOrgId, error: error });
        if (typeof window.toast === 'function') window.toast('Не удалось выполнить действие', 'err');
      }
    });
    modal.__detailsActionsBound = true;
  }
}
window.renderOrganizationDetailsModal = renderOrganizationDetailsModal;

function ensureOrganizationEditModal(){
  var el=document.getElementById('ov-orgEdit');
  if(el) return el;
  el=document.createElement('div');
  el.className='ov';
  el.id='ov-orgEdit';
  el.innerHTML=''
    +'<div class="modal" style="max-width:760px;width:min(760px,calc(100vw - 24px));">'
      +'<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">'
        +'<div>'
          +'<div class="m-title" id="orgEditTitle">✏️ Организация</div>'
          +'<div id="orgEditMeta" style="font-size:12px;color:var(--t2);margin-top:6px;"></div>'
        +'</div>'
        +'<button class="tbBtn" onclick="closeOrganizationEditModal()" style="cursor:pointer;">Закрыть</button>'
      +'</div>'
      +'<div id="orgEditStatus" style="margin-top:12px;font-size:12px;color:var(--t3);"></div>'
      +'<div id="orgEditBody" style="display:grid;gap:12px;margin-top:12px;"></div>'
      +'<div class="m-acts" style="margin-top:18px;">'
        +'<button class="m-ok" data-org-action="save" onclick="submitOrganizationForm()">✅ Сохранить</button>'
        +'<button class="m-cancel" onclick="closeOrganizationEditModal()">Отмена</button>'
      +'</div>'
    +'</div>';
  document.body.appendChild(el);
  return el;
...
function openOrganizationCreateModal(){
  var modal = ensureOrganizationEditModal();
  modal.dataset.mode='create';
  delete modal.dataset.orgId;
  modal.classList.add('on');
  renderOrganizationEditModal(null);
}

function openEditOrganizationModal(orgId){
  logOrganizationAction('edit', orgId);
  var modal = ensureOrganizationEditModal();
  modal.dataset.mode='edit';
  modal.dataset.orgId=String(orgId || '').trim();
  modal.classList.add('on');
  renderOrganizationEditModal(orgId);
}

function renderOrganizationEditModal(orgId){
  var body=document.getElementById('orgEditBody');
  var title=document.getElementById('orgEditTitle');
  var meta=document.getElementById('orgEditMeta');
  var statusEl=document.getElementById('orgEditStatus');
  if(!body || !title || !meta) return;
  var modal=document.getElementById('ov-orgEdit');
  var mode=modal && modal.dataset.mode === 'create' ? 'create' : 'edit';
  var org = mode === 'edit' ? getOrganizationById(orgId) : null;
  title.textContent = mode === 'create' ? '➕ Новая организация' : '✏️ Редактировать организацию';
  meta.textContent = mode === 'create' ? 'Создание новой организации в Supabase' : 'Изменение данных организации';
  if(statusEl) statusEl.textContent = '';
  body.innerHTML=''
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Название</span>'
      +'<input id="orgFormName" class="fS" value="'+_esc((org && org.name) || '')+'" placeholder="Название организации">'
    +'</label>'
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Тип</span>'
      +'<select id="orgFormType" class="fS" style="margin:0;">'
        +['restaurant','bar','cafe','other'].map(function(type){
          var label = type === 'restaurant' ? 'ресторан' : type === 'bar' ? 'бар' : type === 'cafe' ? 'кафе' : 'другое';
          var selected = String((org && org.type) || 'restaurant').toLowerCase() === type;
          return '<option value="'+type+'" '+(selected ? 'selected' : '')+'>'+label+'</option>';
        }).join('')
      +'</select>'
    +'</label>'
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Статус</span>'
      +'<select id="orgFormStatus" class="fS" style="margin:0;">'
        +['active','inactive','archived'].map(function(status){
          var labels = {active:'active',inactive:'inactive',archived:'archived'};
          var selected = String((org && org.status) || 'active').toLowerCase() === status;
          return '<option value="'+status+'" '+(selected ? 'selected' : '')+'>'+labels[status]+'</option>';
        }).join('')
      +'</select>'
    +'</label>'
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Город</span>'
      +'<input id="orgFormCity" class="fS" value="'+_esc((org && org.city) || '')+'" placeholder="Город">'
    +'</label>'
    +'<label style="display:grid;gap:6px;">'
      +'<span style="font-size:12px;color:var(--t2);">Адрес</span>'
      +'<input id="orgFormAddress" class="fS" value="'+_esc((org && org.address) || '')+'" placeholder="Адрес">'
    +'</label>';
  window.__orgEditOrgId = org ? String(org.id || '').trim() : '';
  window.__orgEditMode = mode;
  if (!modal.__editActionsBound) {
    modal.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-org-action="save"]') : null;
      if (!btn || !modal.contains(btn)) return;
      e.preventDefault();
      e.stopPropagation();
      console.info('organization action clicked', { action: 'save', orgId: String(window.__orgEditOrgId || modal.dataset.orgId || '') });
      if (typeof window.submitOrganizationForm === 'function') {
        window.submitOrganizationForm();
      }
    });
    modal.__editActionsBound = true;
  }
}

window.openOrganizationCreateModal = openOrganizationCreateModal;
window.openEditOrganizationModal = openEditOrganizationModal;
window.closeOrganizationEditModal = closeOrganizationEditModal;
window.openOrganizationDetailsModal = openOrganizationDetailsModal;
window.closeOrganizationDetailsModal = closeOrganizationDetailsModal;
window.setOrganizationListFilter = setOrganizationListFilter;
window.openOrganizationDetails = function (orgId) {
  return openOrganizationDetailsModal(orgId);
};
window.openOrganizationEditor = function (orgId) {
  return openEditOrganizationModal(orgId);
};
...
function refreshOrgCachesAfterMutation(orgRow){
  if (!orgRow) return;
  var normalized = {
    id: String(orgRow.id || orgRow.organization_id || '').trim(),
    name: orgRow.name || '',
    type: orgRow.type || 'restaurant',
    status: orgRow.status || 'active',
    city: orgRow.city || '',
  ...
  if (typeof window.refreshOrganizationsForRestaurants === 'function') {
    window.refreshOrganizationsForRestaurants('active').catch(function () {});
    window.refreshOrganizationsForRestaurants('archived').catch(function () {});
    window.refreshOrganizationsForRestaurants('deleted').catch(function () {});
  }
  var detailsModal = document.getElementById('ov-orgDetails');
  if (detailsModal && String(detailsModal.dataset.orgId || '').trim() === normalized.id && typeof renderOrganizationDetailsModal === 'function') {
    renderOrganizationDetailsModal(normalized.id);
  }
  if (typeof renderRestaurants === 'function') renderRestaurants();
}
...
window.submitOrganizationForm = async function () {
  ...
  var mode=modal.dataset.mode === 'create' ? 'create' : 'edit';
  var orgId = String(window.__orgEditOrgId || modal.dataset.orgId || '').trim();
  var name = String((document.getElementById('orgFormName') || { value: '' }).value || '').trim();
  var type = String((document.getElementById('orgFormType') || { value: 'restaurant' }).value || 'restaurant').trim();
  var status = String((document.getElementById('orgFormStatus') || { value: 'active' }).value || 'active').trim();
  var city = String((document.getElementById('orgFormCity') || { value: '' }).value || '').trim();
  var address = String((document.getElementById('orgFormAddress') || { value: '' }).value || '').trim();
  var statusEl=document.getElementById('orgEditStatus');
  var okBtn=modal.querySelector('.m-ok');
  try {
    logOrganizationAction(mode === 'create' ? 'create' : 'edit', orgId);
    if (!name) throw new Error('Укажите название организации');
    if (!window.ownerCreateOrganization || !window.ownerUpdateOrganization) {
      throw new Error('RPC для организаций недоступен');
    }
    if (okBtn) { okBtn.disabled = true; okBtn.textContent = 'Сохраняем...'; }
    if (statusEl) statusEl.textContent = 'Сохранение...';
    var result = null;
    if (mode === 'create') {
      result = await window.ownerCreateOrganization({
        target_name: name,
        target_type: type,
        target_city: city,
      ...
      if (!orgId) throw new Error('Организация не выбрана');
      result = await window.ownerUpdateOrganization({
        target_organization_id: orgId,
        target_name: name,
        target_type: type,
    ...
    if (!result || !result.id) throw new Error('Не удалось сохранить организацию');
    var confirmation = await window.ownerListOrganizations(status === 'deleted' ? 'deleted' : (status === 'archived' ? 'archived' : 'active')).catch(function () { return []; });
    var confirmRow = Array.isArray(confirmation) ? confirmation.find(function (item) { return String(item.id || '') === String(result.id || ''); }) : null;
    if (!confirmRow) {
      confirmRow = {
        id: result.id,
    ...
    if (mode === 'create' && typeof window.setActiveOrganization === 'function') {
      try {
        window.setActiveOrganization(result.id);
      } catch (switchError) {}
    }
    closeOrganizationEditModal();
    if (typeof window.toast === 'function') window.toast(mode === 'create' ? 'Организация создана' : 'Организация сохранена', 'ok');
    return true;
  } catch (error) {
    console.error('submitOrganizationForm failed:', error);
    if (statusEl) statusEl.textContent = error && error.message ? error.message : 'Не удалось сохранить организацию';
    if (typeof window.toast === 'function') window.toast(error && error.message ? error.message : 'Не удалось сохранить организацию', 'err');
    return false;
  } finally {
    if (okBtn) { okBtn.disabled = false; okBtn.textContent = '✅ Сохранить'; }
  }
};

window.archiveOrganizationFromCard = async function (orgId) {
  try {
    logOrganizationAction('archive', orgId);
    if (!confirm('Архивировать организацию? Пользователи больше не смогут работать в ней, но данные сохранятся.')) return false;
    ...
    if (typeof window.toast === 'function') window.toast('Не удалось архивировать организацию', 'err');
    return false;
  }
};
window.restoreOrganizationFromCard = async function (orgId) {
  try {
    logOrganizationAction('restore', orgId);
    ...
    if (typeof window.toast === 'function') window.toast('Не удалось восстановить организацию', 'err');
    return false;
  }
};
window.deleteOrganizationFromCard = async function (orgId) {
  try {
    logOrganizationAction('delete', orgId);
    if (!confirm('Удалить организацию? Она будет скрыта из платформы. Данные не будут физически удалены.')) return false;
    var result = await window.ownerDeleteOrganization(orgId);
    if (!result || !result.id) throw new Error('Не удалось удалить организацию');
    removeOrganizationFromSession(orgId);
    if (typeof window.refreshOrganizationsForRestaurants === 'function') {
      await window.refreshOrganizationsForRestaurants('deleted').catch(function () {});
      await window.refreshOrganizationsForRestaurants('active').catch(function () {});
    }
    if (typeof renderRestaurants === 'function') renderRestaurants();
    ...
    if (typeof window.toast === 'function') window.toast('Не удалось удалить организацию', 'err');
    return false;
  }
};
...
function openSupPriceUpload(supName, append){
    ...
    orgSelect.onchange = function(){
      ...
      if (_supPriceOrganizationId && typeof window.ownerListLegalEntities === 'function') {
        ...
        Promise.resolve(window.ownerListLegalEntities(_supPriceOrganizationId)).then(function (result) {
          var loaded = Array.isArray(result && result.data)
            ? result.data
            : Array.isArray(result)
              ? result
              : [];
          window.__supPriceLegalEntities = loaded.slice();
          window.__supPriceLegalEntitiesLoading = false;
          console.info('supplier price legal entities loaded', {
            organizationId: _supPriceOrganizationId,
            count: window.__supPriceLegalEntities.length,
            legalEntities: window.__supPriceLegalEntities
          });
          if (window.__userSession) {
            window.__userSession.legalEntities = loaded.slice();
          }
          renderSupPriceLegalList(_supPriceOrganizationId, dbGet());
        }).catch(function (error) {
          window.__supPriceLegalEntities = [];
  ...
  if (_supPriceOrganizationId && typeof window.ownerListLegalEntities === 'function') {
    ...
    Promise.resolve(window.ownerListLegalEntities(_supPriceOrganizationId)).then(function (result) {
      var items = Array.isArray(result && result.data)
        ? result.data
        : Array.isArray(result)
          ? result
          : [];
      window.__supPriceLegalEntities = items.slice();
      window.__supPriceLegalEntitiesLoading = false;
      console.info('supplier price legal entities loaded', {
        organizationId: _supPriceOrganizationId,
        count: window.__supPriceLegalEntities.length,
...
function setSupplierPriceManagerTab(tab){
  _supPriceManagerTab = String(tab || 'lists').trim() === 'catalog' ? 'catalog' : 'lists';
  renderSupplierPriceManager();
}
window.setSupplierPriceManagerTab = setSupplierPriceManagerTab;

function loadSupplierPriceCatalogRows(force){
  var supplier = resolveSupplierPriceContext();
  var orgId = String((supplier && (supplier.organization_id || supplier.organizationId)) || (window.__userSession && window.__userSession.activeOrganizationId) || '').trim();
  if (!supplier || !orgId || typeof window.ownerListSupplierPriceCatalog !== 'function') {
    return Promise.resolve([]);
  }
  if (_supPriceCatalogLoading && !force) {
...
function renderSupplierPriceManagerLists(rows){
  rows = Array.isArray(rows) ? rows : [];
  var supplier = resolveSupplierPriceContext();
  ...
  if (!rows.length) {
    return '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);text-align:center;color:var(--t2);">'
...
function renderSupplierPriceManagerCatalog(rows){
  rows = Array.isArray(rows) ? rows : [];
  var search = String(_supPriceCatalogSearch || '').trim().toLowerCase();
  var filtered = search ? rows.filter(function(row){
    var text = [
      row.raw_name,
      row.normalized_name,
      row.price_list_title,
      row.unit,
      row.currency
  ...
  if (!rows.length) {
    return '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);text-align:center;color:var(--t2);">'
      +'<div style="font-size:18px;font-weight:700;margin-bottom:8px;">Прайс-каталог пока пуст</div>'
      +'<div style="font-size:13px;">Все позиции из main и extra прайсов появятся здесь после импорта.</div>'
    +'</div>';
  }
  if (!filtered.length) {
    return '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);text-align:center;color:var(--t2);">'
      +'<div style="font-size:18px;font-weight:700;margin-bottom:8px;">Ничего не найдено</div>'
      +'<div style="font-size:13px;">Попробуйте другой поисковый запрос.</div>'
    +'</div>';
  }
  return '<div style="overflow:auto;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);">'
...
function renderSupplierPriceManager(){
  ...
  body.innerHTML = ''
    +'<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;">'
      +'<div style="display:flex;gap:8px;flex-wrap:wrap;">'
        +'<button class="tbBtn '+(_supPriceManagerTab === 'lists' ? 'bg' : '')+'" data-sup-price-tab="lists" onclick="setSupplierPriceManagerTab(\'lists\')" style="cursor:pointer;">Список прайсов</button>'
        +'<button class="tbBtn '+(_supPriceManagerTab === 'catalog' ? 'bg' : '')+'" data-sup-price-tab="catalog" onclick="setSupplierPriceManagerTab(\'catalog\')" style="cursor:pointer;">Все позиции</button>'
      +'</div>'
      +(_supPriceManagerTab === 'catalog'
        ? '<input type="search" value="'+_esc(_supPriceCatalogSearch || '')+'" placeholder="Поиск по товару..." oninput="window.__supPriceCatalogSearch=this.value||\'\';renderSupplierPriceManager()" style="min-width:240px;flex:1;max-width:420px;padding:10px 12px;border:1px solid var(--br);border-radius:12px;background:#fff;color:var(--tx);">'
        : '<div style="font-size:12px;color:var(--t3);">Выберите режим просмотра прайсов</div>')
    +'</div>'
    +'<div id="supPriceManagerView" style="display:grid;gap:12px;"></div>';
  var view = document.getElementById('supPriceManagerView');
  var orgId = String((supplier && (supplier.organization_id || supplier.organizationId)) || (window.__userSession && window.__userSession.activeOrganizationId) || '').trim();
  if (!view) return;
  if (_supPriceManagerTab === 'catalog') {
    if (typeof window.ownerListSupplierPriceCatalog !== 'function') {
      view.innerHTML = '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);color:var(--t2);">Каталог позиций недоступен.</div>';
      return;
    }
    view.innerHTML = _supPriceCatalogLoading
      ? '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);color:var(--t2);">Загрузка всех позиций...</div>'
      : renderSupplierPriceManagerCatalog(_supPriceCatalogRows);
    if (!_supPriceCatalogLoading && !_supPriceCatalogLoaded) {
      _supPriceCatalogLoading = true;
      view.innerHTML = '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);color:var(--t2);">Загрузка всех позиций...</div>';
      loadSupplierPriceCatalogRows(true).then(function(rows){
        if (String(_supPriceManagerTab) !== 'catalog') return;
        view.innerHTML = renderSupplierPriceManagerCatalog(rows);
      }).catch(function(error){
        if (String(_supPriceManagerTab) !== 'catalog') return;
        view.innerHTML = '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);text-align:center;color:var(--t2);">'
          +'<div style="font-size:18px;font-weight:700;margin-bottom:8px;">Не удалось загрузить каталог прайса</div>'
          +'<div style="font-size:13px;white-space:pre-wrap;">'+_esc((error && error.message) || 'Ошибка загрузки')+'</div>'
  ...
  if (typeof window.ownerListSupplierPriceLists !== 'function') {
    view.innerHTML = '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);color:var(--t2);">Список прайсов недоступен.</div>';
    return;
  }
  view.innerHTML = '<div style="padding:24px;border:1px solid rgba(148,163,184,.16);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08);color:var(--t2);">Загрузка прайс-листов...</div>';
  window.ownerListSupplierPriceLists({
    target_supplier_id: supplier._id || supplier.id || supplier.supplier_id || supplier.supplierId,
    target_organization_id: orgId
  }).then(function(rows){
    rows = Array.isArray(rows) ? rows : [];
    view.innerHTML = renderSupplierPriceManagerLists(rows);
  }).catch(function(error){
...
function renderSupPriceLegalList(orgId, db){
  ...
  var legalEntities = Array.isArray(window.__supPriceLegalEntities) ? window.__supPriceLegalEntities.slice() : [];
  if (window.__supPriceLegalEntitiesLoading) {
    listEl.innerHTML = '<div style="color:var(--t3);padding:8px;font-size:12px;">Загрузка юр. лиц...</div>';
    setSupplierPriceSaveEnabled(false, 'Загрузка юрлиц...');
    return;
  ...
  if(!legalEntities.length){
    if(window.ownerListLegalEntities && resolvedOrgId && !window.__loginInProgress && !window.__restoreInProgress){
      listEl.innerHTML = '<div style="color:var(--t3);padding:8px;font-size:12px;">Загрузка юр. лиц...</div>';
      setSupplierPriceSaveEnabled(false, 'Загрузка юрлиц...');
      Promise.resolve(window.ownerListLegalEntities(resolvedOrgId)).then(function(result){
        var items = Array.isArray(result && result.data)
          ? result.data
          : Array.isArray(result)
            ? result
  ...
  if (legalEntities.length === 1) {
    var onlyId = String(legalEntities[0] && (legalEntities[0].id || legalEntities[0].legal_entity_id || legalEntities[0].legalEntityId || '')).trim();
    if (onlyId) {
      _supPriceLegalEntityIds = [onlyId];
      _supPriceLegalEntityNames = [String(legalEntities[0] && (legalEntities[0].name || legalEntities[0].title) || 'Юрлицо без названия').trim()];
    }
  }
  listEl.innerHTML = legalEntities.map(function(le){
    ...
    var checked = (legalEntities.length === 1) || (Array.isArray(_supPriceLegalEntityIds) && _supPriceLegalEntityIds.indexOf(legalId) >= 0);
    return '<label style="display:flex;align-items:flex-start;gap:10px;padding:6px 4px;cursor:pointer;">'
      +'<input type="checkbox" class="sup-price-comp-cb" value="'+_esc(legalId)+'" data-legal-name="'+_esc(legalName)+'"'+(checked ? ' checked' : '')+' '
  ...
  syncSupPriceLegalSelection();
  if (legalEntities.length === 1) {
    console.info('supplier price legal entities auto selected', {
      organizationId: resolvedOrgId,
      selectedId: _supPriceLegalEntityIds[0] || null,
